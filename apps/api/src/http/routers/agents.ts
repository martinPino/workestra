 ...
 *  3. **El workspace sale del principal**, nunca de la petición: `ws(c)` es el
 *     equivalente exacto del decorador `@Workspace()`.
 */
export function agentsRouter(): Hono<RouterEnv> {
  const r = new Hono<RouterEnv>();

  r.get('/', (c) => c.json(c.get('services').agents.list(ws(c))));
  ...
  r.post('/chat', requireScopes('agent:write'), async (c) => {
    ...
    return c.json(await c.get('services').agents.chatDraft(body?.message ?? '', ws(c), { model: body?.model }));
  });
  ...
  r.post('/mcp/verify', async (c) => {
    ...
    return c.json(await c.get('services').agents.verifyMcp(body?.url ?? '', ws(c), body?.serverId));
  });
  ...
  r.post('/mcp/connect', requireScopes('agent:write'), async (c) => {
    ...
    return c.json(await c.get('services').agents.connectMcp(ws(c), body?.serverId ?? '', body?.token ?? ''));
  });

  r.delete('/mcp/connect/:serverId', requireScopes('agent:write'), async (c) =>
    c.json(await c.get('services').agents.disconnectMcp(ws(c), c.req.param('serverId'))),
  );

  r.get('/:id', async (c) => c.json(await c.get('services').agents.get(c.req.param('id'), ws(c))));

  r.post('/', requireScopes('agent:write'), async (c) =>
    c.json(await c.get('services').agents.create(await c.req.json(), ws(c))),
  );

  r.patch('/:id', requireScopes('agent:write'), async (c) =>
    c.json(await c.get('services').agents.update(c.req.param('id'), await c.req.json(), ws(c))),
  );

  r.delete('/:id', requireScopes('agent:write'), async (c) =>
    c.json(await c.get('services').agents.remove(c.req.param('id'), ws(c))),
  );
