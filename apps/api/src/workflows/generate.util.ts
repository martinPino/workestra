...
export function buildGeneratePrompt(connectors: GenConnector[], agents: GenAgent[]): string {
  ...
  return [
    ...
    '- "extract": {"fileId":"{{file:descargar.id}}"}  → «Extraer texto»: lee un fichero (referencia {{file:KEY.id}} de un paso "download") y saca su texto: PDF de texto, IMÁGENES (foto/escaneo de una factura o recibo → OCR), CSV, TXT. El resultado va a {{text:KEY.text}} para que un agente/LLM lo procese (p. ej. extraer los datos de una factura).',
